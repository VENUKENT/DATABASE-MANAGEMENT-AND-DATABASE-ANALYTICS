...
/*
The drop table statements below remove the current copies of these tables so they 
can be re-created.
If you do not remove previous versions the new CREATE TABLE statement will give you
a 'name is already used' type error.
IMPORTANT - These statements will give you an error if run in your database before 
the tables are created
in your database.  Just ignore these 'table does not exist' errors(arising from 
these statements only).  
They do no damage and will stop when
you finish your script by inserting the missing create table statements. When you 
are FINISHED, this script should 
run with no errors - but not until you are finished.  */
/*
If you run this script before creating any other tables it will NOT create any of 
the four tables
that have the code included.  This is because each of these four tables has a 
foreign key that is
the primary key of a table that is not included.  For intance the P_INCIDENT table 
has a foreign key from the P_DOG 
table.  Once you write the code for P_DOG and create it, then P_INCIDENT will be 
able to be created.
*/
/* 
The drop table and create table statements are in the order they need to be to drop
and create properly.
If you change the order, then errors may occur because dropping or creating in a 
different order may
...
DOG_NAME                VARCHAR2(30 BYTE)    DEFAULT 'UNKNOWN'   NOT NULL,
DOG_BIRTH_MONTH         NUMBER(2)                                NULL,
DOG_BIRTH_YEAR          NUMBER(4)                                NOT NULL,
SEX                     CHAR(1 BYTE)                             NOT NULL,
SPAYED_OR_NEUTERED      CHAR(1 BYTE)         DEFAULT 'N'         NOT NULL,
...
BREED_ID            CHAR(5 BYTE)                 NOT NULL,
BREED_COMMON_NAME   VARCHAR2(60 BYTE)            NOT NULL,
...
OWNER_ID            CHAR(5 BYTE)                                     NOT NULL,
OWNER_FNAME         VARCHAR2(40 BYTE)                                NULL,
OWNER_LNAME         VARCHAR2(60 BYTE)                                NOT NULL,
STREETADDRESS1      VARCHAR2(60 BYTE)                                NULL,
STREETADDRESS2      VARCHAR2(60 BYTE)                                NULL,
CITY                VARCHAR2(60 BYTE)           DEFAULT'LARKSVILLE'  NULL,
STATE               CHAR(2 BYTE)                DEFAULT 'PA'         NULL,
ZIP                 CHAR(5 BYTE)                DEFAULT '18704'      NULL,
TIMESTAMP           TIMESTAMP(2)                DEFAULT SYSDATE     NOT NULL,
PHONE               CHAR(10 BYTE)                                    NULL,
...
CREATE TABLE P_PET (
DOG_ID                          CHAR (5 BYTE)                              NOT NULL,
PET_DOG_LICENSE_NUM             CHAR(10 BYTE)      DEFAULT 'UNLICENSED'    NOT NULL,
